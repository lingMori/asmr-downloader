...
// dev 经 vite proxy 同源转发 /api 与 /media(见 vite.config.ts server.proxy),
// 生产由后端同包伺服,默认恒为相对路径;VITE_API_BASE_URL 可覆盖为绝对地址。
const API_BASE = import.meta.env.VITE_API_BASE_URL || "/api";
// API_BASE 为 "/api" 时 SERVER_BASE 为空串,toAbsoluteMediaUrl 保持 /media/... 等相对路径不变。
